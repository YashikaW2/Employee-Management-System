...
<head>
    ...
    <style>
        ...
        body {
            background: url("img/new/home/image1.jpg");
...
<div style="text-align:centerc;color: white">
    ...
    <form method="POST">
        <div style="text-align: right">
            <label>Search: </label>
            <input type="text" placeholder="Type To Search" name="key" autocomplete="off"
                   style="border-right:none; border-left: none; border-top:none;outline: none;border-bottom: 1px solid white;background-color:transparent; color: black">
...
<table style="width:100%;">
    ...
    if (isset($_POST['sub'])) {
        $link = mysqli_connect("localhost", "root", "19960913", "project") or die($link);
        $k = $_POST['key'];
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr><form action='viewempprof.php' method='post'>
                    ...
                    $row['uname'] . "</td><td>" . $row['doj'] . "</td><td>" . $row['mobile'] . "</td>
						<td>" .
                    "<button type='submit' class='btn btn-xs btn-success' name='_view'>View Profile</button></form> <br> <br>
							<form action='' method='POST'> 
							<input type='text' style='display:none;' name='IDD' value=" . $row['uname'] . ">
							<button type='submit' class='btn btn-xs btn-danger' name='del_emp'>Delete Employee Data</button>
							</form></td></tr>";
                if (isset($_POST['IDD'])) {
                    $name = $_POST['IDD'];
